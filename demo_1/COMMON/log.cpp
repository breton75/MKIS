...
void log_ns::log(int mode, QString text, QWidget *parent)
{
  switch (mode) {
    ...
    case m_Attention:
      {
        wLog->setTextColor(Qt::darkMagenta);
        break;
      } 
...
void log_ns::log_SQL_error(QSqlError error, QString noErrorMsg)
{
  switch (error.type())
  {
    ...
    case QSqlError::ConnectionError:
      log(m_Critical, QString("Connection Error: %1\n").arg(error.text()));
      break;      
  
    case QSqlError::StatementError:
      log(m_Critical, QString("Statement Error: %1\n").arg(error.text()));
      break;  
      
    case QSqlError::TransactionError:
      log(m_Critical, QString("Transaction Error: %1\n").arg(error.text()));
      break;      
      
    case QSqlError::UnknownError:
      log(m_Critical, QString("Error: %1\n").arg(error.text()));
      break;       
  }
}
