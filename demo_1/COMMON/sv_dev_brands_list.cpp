...
SvDevBrandsListDialog::SvDevBrandsListDialog(QWidget *parent,
  ...
  p_brands_model(new QSqlQueryModel(this))
{
...
//  QDialog::accept();
  ...
  QSqlRecord r = p_brands_model->record(Dialog->tblBrandsList->currentIndex().row());
...
//  return 0;
  ...
  QSqlError err = PGDB->execSQL(D_SELECT_DEV_BRANDS_LIST, p_brands_model);
  if(err.type() != QSqlError::NoError) return -1;
  
  Dialog->tblBrandsList->setModel(p_brands_model);
  
  QStringList headers = QString(D_DEV_BRANDS_COLUMN_LIST).split(';');
  
  for(int i = 0; i < headers.count(); i++)
    p_brands_model->setHeaderData(i,  Qt::Horizontal, headers.at(i));
