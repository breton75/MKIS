...
//  t_libPath = model->data(model->index(Dialog->tblModelsList->selectionModel()->currentIndex().row(), 3)).toString();
    ...
    QSqlRecord r = model->record(Dialog->tblModelsList->currentIndex().row());
    t_id = r.value("id").toInt();
    t_modelName = r.value("model_name").toString();
    t_brandName = r.value("model_brand").toString();
    t_className = r.value("model_class").toString();
...
//    Dialog->tblModelsList->setColumnWidth(i, 250);
    ...
    QSqlError err = PGDB->execSQL(D_SELECT_DEV_MODELS_LIST, model);
    if(err.type() != QSqlError::NoError) return -1;

    Dialog->tblModelsList->setModel(model);

    QStringList headers = QString(D_DEV_MODELS_COLUMN_LIST).split(';');
