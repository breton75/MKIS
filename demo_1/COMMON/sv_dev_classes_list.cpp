...
SvDevClassesListDialog::SvDevClassesListDialog(QWidget *parent, bool showEditDelete, bool showSelect) :
  ...
  p_classes_model(new QSqlQueryModel(this))
{
...
//  t_libPath = model->data(model->index(Dialog->tblClassesList->selectionModel()->currentIndex().row(), 1)).toString();
    QSqlRecord r = p_classes_model->record(Dialog->tblClassesList->currentIndex().row());
    t_id= r.value("id").toInt();
    t_className=r.value("class_name").toString();
    t_libPath=r.value("class_lib_path").toString();
...
//    Dialog->tblClassesList->setColumnWidth(i, 250);
    ...
    QSqlError err = PGDB->execSQL(D_SELECT_DEV_CLASSES_LIST, p_classes_model);
    if(err.type() != QSqlError::NoError) return -1;

    Dialog->tblClassesList->setModel(p_classes_model);

    QStringList headers = QString(D_DEV_CLASSES_COLUMN_LIST).split(';');

    for(int i = 0; i < headers.count(); i++)
      p_classes_model->setHeaderData(i,  Qt::Horizontal, headers.at(i));
