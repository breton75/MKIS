  ...
  Dialog(new Ui::SvDeviceDialog)
{
  ...
  if(showMode == smEdit)
  {
    ...
    QSqlError err = PGDB->execSQL(QString(D_SELECT_FROM_DEVICES_WHERE_2).arg("device_name").arg("'" + deviceName + "'"), q);
    ...
    t_className = q->value("device_class").toString();
    t_brandName = q->value("device_brand").toString();
    t_modelName = q->value("device_model").toString();
    t_ip = q->value("ip").toString();
    t_port = q->value("port").toInt();
    t_driverPath = q->value("driver_lib_path").toString();
    t_videoPath  = q->value("video_path").toString();
    t_zoneName = q->value("zone_name").toString();
    t_X = q->value("pos_x").toFloat();
    t_Y = q->value("pos_y").toFloat();
...
void SvDevice::accept()
{
  ...
  t_driverPath = Dialog->editDriver->text();
...
//      }
//      else groupName = 'a';
      s = QString(D_INSERT_INTO_DEVICES)
              .arg(t_deviceName)
              .arg(t_modelName)
              .arg(t_className)
              .arg(t_brandName)
...
//        }
//      }
      s = QString(D_UPDATE_DEV_DEVICES)
              .arg(t_deviceName)
              .arg(t_modelName)
              .arg(t_className)
              .arg(t_brandName)
